...
def _get_date_context() -> str:
    """Always inject current date/time so Gemini never hallucinates dates."""
    now = datetime.now()
    return f"""
...
async def parse_opportunity_from_text(text: str) -> dict:
    """
    User pastes any opportunity text with 'opp' keyword.
    Extracts everything: title, type, deadline, notes, link.
...
Koded pasted this opportunity text:
\"\"\"{text}\"\"\"

Extract ALL relevant information. Be smart about finding:
- title: the name of the opportunity
- type: hackathon | internship | deadline | event | grant | general
- deadline: find ANY date mentioned (application deadline, submission date, event date) → format as YYYY-MM-DD. If multiple dates, use the earliest/most critical one.
- notes: key details (prize, requirements, eligibility, location, what it's for)
...
{{
    ...
    try:
        ...
        logger.error(f"Opp parse error: {e}")
        return {"title": text[:80], "type": "general", "deadline": None, "notes": None, "link": None}


async def parse_opportunity_from_text(text: str) -> dict:
    """
    Parse a pasted opportunity description and extract structured data.
    Can handle raw copy-paste from emails, tweets, Notion pages, anywhere.
    Returns:
    {
        "title": "...",
        "type": "hackathon|internship|deadline|event|grant|competition|general",
        "deadline": "YYYY-MM-DD or null",
        "link": "https://... or null",
        "notes": "one line summary of what this is"
    }
    """
    prompt = f"""{KODED_CONTEXT}
